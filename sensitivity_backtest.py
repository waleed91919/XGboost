...
# Thresholds von 0.55 bis 0.65 in 0.01er Schritten
THRESHOLDS = [round(x * 0.01, 2) for x in range(55, 66)]

def calculate_features(df):
    """Exakt dieselbe Feature-Logik wie in long_term_backtest.py."""
    df = df.copy()
...
def run_sensitivity_analysis():
    ...
    df['prob'] = model.predict_proba(X)[:, 1] # Wahrscheinlichkeit als 0.0 bis 1.0

    results = []

    print(f"\n🚀 Starte Sensitivitätsanalyse (V1 Basis-Modell)...")
    print("-" * 65)
    print(f"{'Threshold':>10} | {'PnL %':>12} | {'Win-Rate':>10} | {'Trades':>8}")
    print("-" * 65)
    ...
    for threshold in THRESHOLDS:
        ...
        # In der Schleife nutzen wir threshold als Wahrscheinlichkeit (0.55 bis 0.65)
        ...
        print(f"{threshold:>10.2f} | {total_pnl_pct:>11.2f}% | {win_rate:>9.2f}% | {trades_count:>8}")
    ...
    if results:
        ...
        print(f"🏆 Bestes Ergebnis bei Threshold {best['Threshold']:.2f} mit {best['PnL_Pct']:.2f}% Profit!")
